...
export function NotificationList({ initialNotifications }: { initialNotifications: NotificationItem[] }) {
    ...
    const loadMore = async () => {
        ...
        // Load 10 more
        const newItems = await getNotificationHistory(offset, 10)
                    ...
                    {hasMore && (
                        <div className="pt-4 flex justify-center">
                            <motion.button
                                whileTap={{ scale: 0.95 }}
                                onClick={loadMore}
                                onViewportEnter={() => loadMore()} // Simple infinite scroll
                                className="text-xs font-medium text-zinc-400 flex items-center gap-2 px-4 py-2 hover:bg-zinc-50 rounded-full transition-colors"
                            >
                                {loading ? (
                                    <>
