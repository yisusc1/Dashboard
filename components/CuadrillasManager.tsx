...
import { Plus, Trash2, Edit2, Loader2, Save, X, UserPlus } from "lucide-react"
...
export default function CuadrillasManager() {
  ...
  return (
    <div className="space-y-6">
        ...
        {/* HEADER */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-[24px] border border-zinc-200 shadow-sm gap-4">
            <div>
            ...
            <div className="flex gap-2">
                {!isAddingTecnico && (
                    ...
                    >
                        <UserPlus size={18} /> Registrar Técnico
                    </button>
                )}
                {!isAdding && (
                    <button 
        ...
        {/* REGISTRAR TÉCNICO FORM */}
        {isAddingTecnico && (
            <div className="bg-emerald-50 border border-emerald-200 p-6 rounded-[24px] shadow-sm flex flex-col sm:flex-row gap-4 items-end">
                <div className="flex-1">
                    <label className="block text-sm font-semibold text-emerald-900 mb-1">Nombre del Técnico *</label>
                    <input 
                        type="text" 
                        value={newTecnicoNombre} 
                        onChange={e => setNewTecnicoNombre(e.target.value)} 
                        className="w-full h-10 px-3 rounded-xl border border-emerald-200 focus:ring-2 focus:ring-emerald-500 outline-none"
                        placeholder="Ej. Douglas Mayobre"
                        autoFocus
                    />
                </div>
                <div className="flex gap-2 w-full sm:w-auto">
                    <button onClick={handleAddTecnico} className="h-10 px-4 bg-emerald-600 text-white rounded-xl font-medium hover:bg-emerald-700 flex-1 sm:flex-none flex items-center justify-center gap-2"><Save size={16}/> Guardar</button>
                    <button onClick={() => setIsAddingTecnico(false)} className="h-10 px-4 bg-white text-zinc-600 border border-zinc-200 rounded-xl hover:bg-zinc-50 flex items-center justify-center"><X size={16}/></button>
                </div>
            </div>
        )}
                    ...
                    {editingId === c.id ? (
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 flex-1">
                            ...
                            <div className="flex items-end gap-2">
                                <button onClick={handleSaveEdit} className="h-10 px-4 bg-violet-600 text-white rounded-xl hover:bg-violet-700 flex-1 flex items-center justify-center gap-2"><Save size={16}/> Guardar</button>
