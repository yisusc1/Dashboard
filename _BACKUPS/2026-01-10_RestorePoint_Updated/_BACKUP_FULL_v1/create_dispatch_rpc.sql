...
BEGIN
    ...
    LOOP
        ...
        -- Note: v_item->'serials' comes as ["A", "B"], we need to cast to text[]
        ...
        -- Check for negative stock? Optional constraints.

        -- C. Create Transaction Record (Audit Log)
        INSERT INTO inventory_transactions (
            product_id,
            type,
            quantity,
            previous_stock, -- We could fetch this, but for opt performance maybe skip or do subquery
            new_stock,
            reason,
            assigned_to,
            serials,
        ...
        SELECT 
            ...
            'OUT', -- Dispatch Type
            v_quantity,
            p.current_stock + v_quantity, -- Reconstruct previous
            p.current_stock,
            'Despacho a Técnico',
            p_assigned_to,
            to_jsonb(v_serials),
