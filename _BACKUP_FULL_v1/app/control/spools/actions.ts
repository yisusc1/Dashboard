...
export async function getActiveSpools() {
    ...
    // Enhance with current status from Serial Master Record
    const enriched = await Promise.all(data.map(async (a: any) => {
        ...
        // Calculate Usage since Assignment Created
        // We sum 'metraje_usado' + 'metraje_desechado' from cierres 
        // where codigo_carrete = serialNum AND created_at >= assignment.created_at
        const { data: usageData } = await supabase
            .from("cierres")
            .select("metraje_usado, metraje_desechado")
            .eq("codigo_carrete", serialNum)
            .gte("created_at", a.created_at) // Only usage AFTER assignment

        let totalUsage = 0
        usageData?.forEach((u: any) => {
            const uVal = Number(String(u.metraje_usado).replace(/[^0-9.]/g, "")) || 0
            const wVal = Number(String(u.metraje_desechado).replace(/[^0-9.]/g, "")) || 0
            totalUsage += (uVal + wVal)
        })

        const masterCurrent = serialData?.current_quantity || item.quantity
        // The "Current" displayed should be Master (Start of Shift/Audit) - Usage Since Then?
        // OR: Master Record should be updated daily? 
        // Logic: Master Record holds Validated Stock. Usage is "Pending Deduction".
        // Display: Estimated Current = Master - Usage.

        return {
            ...a,
            serial_number: serialNum,
            product_name: item.product?.name || "Desconocido", // Safe access
            initial_quantity: serialData?.initial_quantity || item.quantity,
            current_quantity: masterCurrent - totalUsage, // Dynamic Estimate
            reported_usage: totalUsage // Useful for debug
        }
    }))
